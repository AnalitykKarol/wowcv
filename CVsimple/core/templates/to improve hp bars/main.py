...
def check_dependencies():
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            print(f"💻 CUDA: Niedostępne (CPU only)")
        dep_status['torch'] = True
    except ImportError:
        print(f"⚠️ torch: BRAK (opcjonalne dla YOLOv8)")
    ...
    print("✅ Wszystkie zależności spełnione!")
...
def test_imports():
    ...
    modules_to_test = [
        ...
        ('core.hp_bar_analyzer', 'PlayerHPBarAnalyzer')  # ← NOWY: HP Analyzer
    ]
    ...
    for module_path, class_name in modules_to_test:
        try:
            ...
            print(f"❌ {module_path}.{class_name}: BŁĄD - {str(e)}")
            import_results[f"{module_path}.{class_name}"] = False
            # Dla HP analyzer - pokaż szczegóły błędu
            if 'hp_bar_analyzer' in module_path:
                print(f"   💡 Upewnij się że plik hp_bar_analyzer.py istnieje i zawiera PlayerHPBarAnalyzer")
            traceback.print_exc()
...
def test_hp_analyzer():
    ...
    print("\n=== TEST HP ANALYZER ===")
    try:
        from core.hp_bar_analyzer import PlayerHPBarAnalyzer

        # Test tworzenia obiektu
        analyzer = PlayerHPBarAnalyzer()
        print("✅ PlayerHPBarAnalyzer: Obiekt utworzony")

        # Test podstawowych metod
        analyzer.configure_screen_size(1920, 1080)
        print("✅ configure_screen_size: OK")

        analyzer.set_manual_position(5, 5, 15, 3)
        print("✅ set_manual_position: OK")

        stats = analyzer.get_stats()
        print(f"✅ get_stats: OK (total_analyses: {stats.get('total_analyses', 0)})")

        return True

    except Exception as e:
        print(f"❌ HP Analyzer test failed: {str(e)}")
        print("   💡 Sprawdź czy hp_bar_analyzer.py zawiera kod PlayerHPBarAnalyzer")
        traceback.print_exc()
        return False
...
def main():
    ...
    try:
        ...
        if not hp_test_ok:
            print("⚠️ HP Analyzer nie przeszedł testów - funkcja może być ograniczona")

        # Importuj komponenty
        try:
            from gui.main_window import MainWindow
            from app_utils.logger import Logger
            print("✅ Główne moduły zaimportowane")
        except Exception as e:
            print(f"❌ Krytyczny błąd importu: {str(e)}")
            traceback.print_exc()
        ...
        try:
            ...
            print(f"⚠️ Nie można utworzyć loggera: {str(e)}")
        ...
        try:
            ...
            if 'hp_bar_analyzer' in str(e).lower() or 'PlayerHPBarAnalyzer' in str(e):
                additional_info = "\n\n💡 Może to być problem z HP Analyzer.\nSprawdź czy plik hp_bar_analyzer.py zawiera PlayerHPBarAnalyzer"
        ...
        try:
            ...
            print("🎮 Sprawdź okno aplikacji")
            print("💚 HP Analyzer: Emergency heal przy HP < 50%")
